...
class MockSagas:
    ...
    @staticmethod
    def update_saga(game_id, saga_data_or_messages, game_active=None):
        """Mock update_saga - handles both calling patterns"""
        if game_active is not None:
            # Called with (game_id, messages, game_active)
            messages = saga_data_or_messages
            if game_id not in MOCK_SAGAS:
                MOCK_SAGAS[game_id] = {
                    'game_id': game_id,
                    'messages': messages,
                ...
                MOCK_SAGAS[game_id]['messages'] = messages
                MOCK_SAGAS[game_id]['game_active'] = game_active
                MOCK_SAGAS[game_id]['updated_at'] = datetime.now().isoformat()
            return MOCK_SAGAS[game_id]
        else:
            # Called with (game_id, saga_data)
            saga_data = saga_data_or_messages
