...
class MockGameManager:
    ...
    @staticmethod
    def create_new_game(user_id, prompt, response, game_name):
        """Mock create_new_game - creates new game with proper initial messages"""
        game_id = str(uuid.uuid4())
        game_data = {
            'game_id': game_id,
            'user_id': user_id,
            'messages': [prompt, response],  # Initial prompt and AI response like real system
            'game_name': game_name,
            'game_active': True,
            'created_at': datetime.now().isoformat(),
            'updated_at': datetime.now().isoformat()
...
class MockUserRecordSchema:
    ...
    @staticmethod
    def add_character(user_data, name, profession, character_id, level):
        ...
        user_data['characters'].append(character_entry)
        return user_data
    
    @staticmethod
    def add_to_active(user_data, character_id, game_id, game_name):
        ...
        user_data['active_games'].append(game_entry)
        return user_data
