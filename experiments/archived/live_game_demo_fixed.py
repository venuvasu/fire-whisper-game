...
FIXED VERSION - Correct XP integration detection
"""
...
class GameSession:
    ...
    def __init__(self):
        self.session_data = {
            'session_info': {
                ...
                'system_version': 'enhanced_v1.0_fixed',
                'character': None,
                'total_turns': 0,
                'total_xp_gained': 0,
                'total_loot_found': 0
    ...
    def add_turn(self, turn_data):
        ...
        # Update XP totals (separate from integration detection)
        ...
        # Enhancement detection (based on narrative integration, not just mechanics)
    ...
    def save_yaml(self, filename=None):
        ...
        if not filename:
            ...
            filename = f"fire_whisper_session_fixed_{timestamp}.yaml.gz"
...
def print_response(response, title="", session=None):
    ...
    turn_data = {
        ...
        'has_xp_integration': False,  # FIXED: Added separate XP integration detection
    ...
    # FIXED: Check for narrative enhancements (separate from mechanical results)
    narrative_lower = narrative.lower()
    
    # Dice integration check - look for natural language integration
    ...
    # XP integration check - look for natural XP language (FIXED)
...
def live_demo(save_yaml=True):
    ...
    print_separator("FIRE WHISPER ENHANCED SYSTEM DEMO - FIXED")
    print("🧚‍♀️ Starting enhanced RPG experience...")
    print("📊 Watch for narrative integration of mechanics!")
    print("🔧 FIXED: Accurate XP integration detection")
    if save_yaml:
    ...
    try:
        ...
        if session:
            ...
            # FIXED: Calculate accurate success rates
            total_action_turns = session.session_data['session_info']['total_turns'] - 1  # Exclude game start
            if total_action_turns > 0:
                dice_rate = (enhancements['dice_integration'] / session.session_data['session_info']['total_turns']) * 100
                xp_rate = (enhancements['xp_integration'] / total_action_turns) * 100 if total_action_turns > 0 else 0
                
                print(f"\n🎯 SUCCESS RATES (FIXED):")
                print(f"   🎲 Dice Integration: {dice_rate:.1f}% ({enhancements['dice_integration']}/{session.session_data['session_info']['total_turns']} responses)")
                print(f"   ✨ XP Integration: {xp_rate:.1f}% ({enhancements['xp_integration']}/{total_action_turns} action turns)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Fire Whisper Live Demo - Fixed Version')
