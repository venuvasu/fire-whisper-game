...
def get_character_by_game_id(user_record, game_id):
    """
    Finds and returns the character that has the specified game_id in either active or completed games.
    
    Args:
        user_record (dict): The user record containing characters
        game_id (str): The game_id to search for
    
    Returns:
        dict: The character object that contains the game_id
    
    Raises:
        ValueError: If no character has the specified game_id or invalid input types
    """
    if not isinstance(user_record, dict):
        raise ValueError("user_record must be a dict")
    if not isinstance(game_id, str):
    ...
    for character in characters:
        ...
        active_games = character.get('active', [])
        for game in active_games:
            if isinstance(game, dict) and game.get('game_id') == game_id:
                return character
        
        # Check in completed games
        completed_games = character.get('completed', [])
        for game in completed_games:
            if isinstance(game, dict) and game.get('game_id') == game_id:
                return character
    
    raise ValueError(f"No character found with game_id '{game_id}'")

def get_character_by_active_game_id(user_record, game_id):
    """
    Finds and returns the character that has the specified game_id in active games only.
    ...
    Returns:
        dict: The character object that contains the game_id in active games
    
    Raises:
        ValueError: If no character has the specified game_id in active games or invalid input types
    """
    if not isinstance(user_record, dict):
        raise ValueError("user_record must be a dict")
    if not isinstance(game_id, str):
    ...
    for character in characters:
        ...
        # Check only in active games
        active_games = character.get('active', [])
        for game in active_games:
            if isinstance(game, dict) and game.get('game_id') == game_id:
                return character
    
    raise ValueError(f"No character found with game_id '{game_id}' in active games")

def get_active_game(user_record, game_id):
    """
    Finds and returns the active game object with the specified game_id.
    ...
    Returns:
        dict: The game object from active games with the matching game_id
    
    Raises:
        ValueError: If no active game found with the specified game_id or invalid input types
    """
    if not isinstance(user_record, dict):
        raise ValueError("user_record must be a dict")
    if not isinstance(game_id, str):
    ...
    raise ValueError(f"No active game found with game_id '{game_id}'")
...
def update_character_level(user_record, character_id, level):
    """
    Updates a character's level in the user record.
    
    Args:
        user_record (dict): The user record containing characters
        character_id (str): The character_id to update
        level (int or Decimal): The new level for the character
    
    Returns:
        dict: The updated user_record
    
    Raises:
        ValueError: If character not found or invalid input types
    """
    if not isinstance(user_record, dict):
        raise ValueError("user_record must be a dict")
    if not isinstance(character_id, str):
